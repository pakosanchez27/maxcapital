...
@section('contenido')
    ...
    <div class="content-wrapper">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-warning"
                                            onclick="editCreditType({{ $type->id }})"><i
                                                class="bi bi-pencil-square"></i></button>
                                        <button class="btn btn-sm btn-danger" onclick="deleteCreditType({{ $type->id }})"><i
        ...
        <!-- Modal -->
        <div class="modal fade" id="showDocuments" tabindex="-1" aria-labelledby="modalScrollableTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalScrollableTitle">Documentos requeridos para <span
                                id="tipo_credito"></span></h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                        </button>
                    </div>
                    <div class="modal-body">
                        <ul id="contenedor_docs" style="list-style-type: disc; margin-left: 20px;"></ul>
        ...
        <div class="modal fade" id="editCreditType" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel1">Edita el Tipo de Credito</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <input type="hidden" id="idCreditType">
                        <div class="row">
                            <div class="col-12 mb-4">
                                <label for="creditTypeName" class="form-label">Tipo de credito</label>
                                <input type="text" id="creditTypeNameEdit" class="form-control" placeholder="Ej. Empresa"
                                    aria-describedby="creditTypeNameEditError">
                                <div id="creditTypeNameEditError" class="form-text text-danger d-none">&nbsp;</div>
                            </div>
                            <div class="col-12 mb-4">
                                <label for="creditTypeDescriptionEdit" class="form-label">Descripción</label>
                                <textarea id="creditTypeDescriptionEdit" class="form-control" rows="3"
                                    placeholder="Descripción del tipo de crédito" aria-describedby="creditTypeDescriptionEditError"></textarea>
                                <div id="creditTypeDescriptionEditError" class="form-text text-danger d-none">&nbsp;</div>

                            </div>
                            <div class="col-12">
                                <label for="select2Multiple_edit" class="form-label">Documentos Rqueridos</label>
                                <select id="select2Multiple_edit" class="select2 form-select" multiple>
                                    @foreach ($documents as $doc)
                                        <option value="{{ $doc->id }}">{{ $doc->name }}</option>
                                    @endforeach
                                </select>
                                <div id="select2Multiple_editError" class="form-text text-danger d-none">&nbsp;</div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">Cerrar</button>
                        <button type="button" class="btn btn-primary" onclick="validateFormCreditTypeEdit()">Actualizar
                            </button>
                    </div>
                </div>
            </div>
    ...
    @section('scripts')
        ...
        <script>
            ...
            $(function() {
                $('#select2Multiple_edit').select2({
                    theme: 'bootstrap-5',
                    placeholder: 'Selecciona uno o varios…',
                    closeOnSelect: false, // no cerrar al elegir (más cómodo para multi)
                    allowClear: true, // muestra botón para limpiar todo
                    dropdownParent: $('#select2Multiple_edit').parent() // evita issues en modales
                    // tags: true, // ← activa si QUIERES permitir escribir etiquetas libres
                });
            });
        </script>
